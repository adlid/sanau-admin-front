...
export const Meters = (props: any) => {
  ...
  return (
    <>
      <div className="gas_create_group__title" style={{ fontSize: "unset", fontWeight: "unset" }}>
        <div className="indication-meters-list__search-block">
          <Search value={searchValue} onChange={(value: string) => setSearchValue(value)} />
          <AppPopper
            gap="1px"
            content={
              <div className="gas_create_group__searchDropDown">
          ...
          <Popover
            ...
            content={
              <div className="power_create_group__popover">
                <Form.Group>
                  <Form.Check
                    checked={items.some((i) => i === "gas-udp")}
                    name="residue"
                    type="checkbox"
                    label="Подключение через UDP
          "
                    onChange={(e) => {
                      if (e.target.checked) setItems([...items, "gas-udp"]);
                      else setItems(items.filter((i) => i !== "gas-udp"));
                    }}
                  />
                </Form.Group>
                ...
                <Form.Group>
                  <Form.Check
                    checked={items.some((i) => i === "gas-lorawan-udp")}
                    name="residue"
                    type="checkbox"
                    label="Подключение через LoRaWAN и UDP"
                    onChange={(e) => {
                      if (e.target.checked) setItems([...items, "gas-lorawan-udp"]);
                      else setItems(items.filter((i) => i !== "gas-lorawan-udp"));
                    }}
                  />
                </Form.Group>
                <div className="mb12px"></div>
                <Form.Group>
                  <Form.Check
                    checked={items.some((i) => i === "gas-web-socket")}
                    name="residue"
                    type="checkbox"
                    label="Подключение через LoRaWAN и WS"
                    onChange={(e) => {
                      if (e.target.checked) setItems([...items, "gas-web-socket"]);
                      else setItems(items.filter((i) => i !== "gas-web-socket"));
                    }}
                  />
                </Form.Group>
                <div className="mb12px"></div>
        ...
        <div className="gas_create_group__table-block">
          <Row className="gas_create_group__table-group">
            <Col
              xl={1}
              lg={1}
              md={4}
              sm={4}
              className="gas_create_group__table-group__column gas_create_group__table-group__column-checkbox"
            >
              <input
                type="checkbox"
            ...
            <Col xl={1} lg={1} md={3} sm={3} className="gas_create_group__table-group__column">
              <span className="gas_create_group__card-title">Сер.№</span>
            </Col>
            <Col xl={3} lg={3} md={3} sm={3} className="gas_create_group__table-group__column">
              <span className="gas_create_group__card-title">Наименование</span>
            </Col>
            <Col xl={3} lg={3} md={3} sm={3} className="gas_create_group__table-group__column">
              <span className="gas_create_group__card-title">Лицевой счет</span>
            </Col>
            <Col xl={4} lg={4} md={4} sm={4} className="gas_create_group__table-group__column">
              <span className="gas_create_group__card-title">ФИО</span>
            </Col>
          </Row>
          <hr className="tableHeaderLine" />

          <div className="gas_create_group__wrap">
            {!metersList?.data.length || metersList === null ? (
              dataLoading ? (
                <div className="gas_create_group__preloader">
                  <Spinner animation="border" size="sm" />
                </div>
              ) : (
                <p className="gas_create_group__preloader">Список пуст</p>
              )
            ) : (
