  ...
  return (
    <>
      <div className="power_create_group__title" style={{ fontSize: "unset", fontWeight: "unset" }}>
          ...
          <Popover
            ...
            content={
              <div className="power_create_group__popover">
                <Form.Group>
                  <Form.Check
                    checked={items.some(i => i === "uspd")}
                    name="residue"
                    type="checkbox"
                    label="Подключение через УСПД
                    "
                    onChange={(e) => {
                      if (e.target.checked) {
                        setItems([...items, "uspd"]);
                      } else {
                        setItems(items.filter(i => i !== "uspd"));
                      }
                    }}
                  />
                </Form.Group>
                ...
                <Form.Group>
                  <Form.Check
                    checked={items.some(i => i === "bluetooth")}
                    name="residue"
                    type="checkbox"
                    label="Подключение через Bluetooth 
                    "
                    onChange={(e) => {
                      if (e.target.checked) {
                        setItems([...items, "bluetooth"]);
                      } else {
                        setItems(items.filter(i => i !== "bluetooth"));
                      }
                    }}
                  />
                </Form.Group>
                ...
                <Form.Group>
                  <Form.Check
                    checked={items.some(i => i === "network")}
                    name="residue"
                    type="checkbox"
                    label="Подключение через LoRaWAN и NS"
                    onChange={(e) => {
                      if (e.target.checked) {
                        setItems([...items, "network"]);
                      } else {
                        setItems(items.filter(i => i !== "network"));
                      }
                    }}
                  />
                </Form.Group>
        ...
        <div className="power_create_group__table-block" style={{ margin: "30px 5px 69px" }}>
          <Row className="power_create_group__table-group">
            <Col
              xl={1}
              lg={1}
              md={4}
            ...
            <Col xl={2} lg={2} md={3} sm={3} className="power_create_group__table-group__column">
              <span className="power_create_group__card-title">Сер.№</span>
            </Col>
            <Col xl={2} lg={2} md={3} sm={3} className="power_create_group__table-group__column">
              <span className="power_create_group__card-title">Наименование</span>
            </Col>
            <Col xl={3} lg={3} md={3} sm={3} className="power_create_group__table-group__column">
              <span className="power_create_group__card-title">Лицевой счет</span>
            </Col>
            <Col xl={4} lg={4} md={4} sm={4} className="power_create_group__table-group__column">
              <span className="power_create_group__card-title">ФИО</span>
            </Col>
          </Row>
          <hr className="tableHeaderLine" />

          <div className="power_create_group__wrap">
            {!metersList?.data.length || metersList === null ? (
              dataLoading ? (
                <div className="power_create_group__preloader">
                  <Spinner animation="border" size="sm" />
                </div>
              ) : (
                <p className="power_create_group__preloader">Список пуст</p>
              )
            ) : (
