...
import { ConcencatorGasCountersChildrenListItem } from "../ConcentratorGasCounterChildrenListItem/ConcentratorGasCounterChildrenListItem";
...
export const ConcentratorGasCountersChildren: FC<IProps> = ({index, keyOfTree}) => {
    // hooks
            ...
            <div className="reports__wrap">
                {!tree[index].children?.length || !tree[index].children ? (
                    ...
                    computePowerCounter!.children.map((item, index) => {
                        return <ConcencatorGasCountersChildrenListItem key={item.key} counterItem={item} index={index + 1}/>
                    })
                )
                }
            </div>
