...
import { addKeyToFolders, removeKeyFromFolder, setSelectedFolderName } from "../../../../../store/slicesAndThunks/reports/reports.slices";
...
export const ConcentratorPowerCounterListItem: FC<PropsType> = (props) => {
  ...
  const handleClick = () => {
    if(!concentratorData.haveParent) {
      dispatch(setSelectedFolderName(concentratorData.title))
    }
  ...
  const computeConcentratorPowerCounterListItem = () => {
    return concentratorData.children && concentratorData.children
    .map((child, i) => <ConcentratorPowerCounterListItem concentratorData={child} key={i} 
  ...
  return (
    <>
      ...
      {
        open && concentratorData && concentratorData.children && computeConcentratorPowerCounterListItem()
      }
