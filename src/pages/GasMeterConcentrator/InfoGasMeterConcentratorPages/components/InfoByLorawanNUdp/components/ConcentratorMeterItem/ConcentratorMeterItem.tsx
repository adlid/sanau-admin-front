...
export const ConcentratorMeterItem: FC<PropsType> = ({ setSelectedMeterObjCallback, meterData, sideBarToggle }) => {
  const history = useHistory();
  const [isSvgHovered, toggleIsSvgHovered] = useState(false);
  const [isPopoverOpen, setIsPopoverOpen] = useState(false);

  return (
    <div className="gas-lorawan-udp-concentrator-meter-item">
      <Row className="gas-lorawan-udp-concentrator-meter-item__header " style={{ wordWrap: "break-word" }}>
        <Col xl={3} lg={3} md={4} sm={4}>
          {meterData.deviceName}
        </Col>
        ...
        <Col xl={1} lg={1} md={4} sm={4} style={{ display: "flex", justifyContent: "flex-end", paddingRight: "35px" }}>
          <Popover
            ...
            positions={["top", "right"]}
            padding={0}
            containerClassName="gas-lorawan-udp-concentrator-popover__container "
            content={
              <div className="gas-lorawan-udp-concentrator-popover">
                <div className="gas-lorawan-udp-concentrator-popover__item" onClick={sideBarToggle(true)}>
                  <InfoIcon />
                  <span>Информация</span>
                </div>
                <div
                  className="gas-lorawan-udp-concentrator-popover__item"
                  onClick={(e) => {
                    history.push(
                      `/admin/concentrators/gas-meter/concentrator-info/edit-udp?meterId=${meterData.id}&meterName=${
                        meterData.deviceName
                      }${
          ...
          >
            <div
              ...
              className={
                isSvgHovered
                  ? "gas-lorawan-udp-concentrator-meter-item__icon gas-lorawan-udp-concentrator-meter-item__icon--hovered d-flex justify-content-center align-items-center"
                  : "gas-lorawan-udp-concentrator-meter-item__icon d-flex justify-content-center align-items-center"
              }
            >
              {meterData.userId ? (
