...
import {
  getXaxisValuesDinRail,
  getYaxisValuesDinRail,
  filteredDinRailList,
...
export const PowerGraphsDinRail: FC<PropsType> = memo(() => {
  const [isBarChart, setIsBarChart] = useState(true);
  const dispatch = useAppDispatch();

  const { DinRailGraphData } = useTypedSelector((state) => state.powerIndication)

  const xAxis = useTypedSelector(getXaxisValuesDinRail);
  const yAxis = useTypedSelector(getYaxisValuesDinRail);

  const deviceIds = useMemo(() => {
    return DinRailGraphData.map((i) => i.deviceName)
  }, [DinRailGraphData]);
    ...
    <div className="meters-graph">
      ...
      <div className="d-flex ">
        <button
          ...
          onClick={() => dispatch(getSelectedGraphNameDinRail("positiveActiveEnergy"))}
        >
          Активная энергия +
        </button>
        ...
        <button
          ...
          onClick={() => dispatch(getSelectedGraphNameDinRail("reverseActiveEnergy"))}
        >
          Активная энергия -
        </button>
        <div className="mr12px"></div>
        <button
          ...
          onClick={() => dispatch(getSelectedGraphNameDinRail("positiveReactiveEnergy"))}
        >
          Реактивная энергия +
        </button>
        <div className="mr12px"></div>
        <button
          ...
          onClick={() => dispatch(getSelectedGraphNameDinRail("reverseReactiveEnergy"))}
        >
          Реактивная энергия -
        </button>
      </div>
