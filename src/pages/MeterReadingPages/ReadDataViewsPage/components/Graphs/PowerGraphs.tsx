...
import { ReadMeterInfoFilterType } from "../../../../../ts/types/indication.types";
...
export const PowerGraphs: FC<PropsType> = memo(({ type = "uspd" }) => {
  ...
  const { selectedGraphNumber, selectedGraphName, metersId, currentUspdMeterId, selectedMeterType, indicationMetersListArr } = useTypedSelector((state) => state.powerIndication);
    ...
    <div className="meters-graph">
      <div className="d-flex justify-content-between">
        <Select
          ...
          value={selectedMeterType === 'uspdMeter' ? currentUspdMeterId : selectedGraphNumber}
          onChange={(e) => {
            if (selectedMeterType === 'uspdMeter') {
              dispatch(setCurrentUspdMeterId(e.target.value as string))
            } else {
        ...
        >
          {indicationMetersListArr?.data.map((item: any) => {
            return (
              <MenuItem key={item} value={item.id.toString()}>
                Прибор учета № {item.serialNumber}
              </MenuItem>
            );
          })}
        </Select>
