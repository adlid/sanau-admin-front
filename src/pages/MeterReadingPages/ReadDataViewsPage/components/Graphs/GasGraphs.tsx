...
import { getXaxisValues, getYaxisValues } from "../../../../../store/selects/gasIndication.selector";

// ts
import { MainGasTableDataType } from "../../../../../ts/types/indication.types";
...
const menuProps: any = {
  ...
  PaperProps: { style: { maxHeight: ITEM_HEIGHT * 4.5 + ITEM_PADDING_TOP, width: "320px" } },
};

interface IGasGraphsProps {
  mainGraphData?: Array<MainGasTableDataType> | null;
}

export const GasGraphs: FC<IGasGraphsProps> = memo((props) => {
  const { mainGraphData } = props;
  const dispatch = useDispatch();

  const [isBarChart, setIsBarChart] = useState(true);

  const xAxis = useTypedSelector(getXaxisValues);
  const yAxis = useTypedSelector(getYaxisValues);

  const { selectedGraphItem, selectedGraphName } = useTypedSelector((state) => state.gasIndication);
  ...
  const getFilteredArray = () => {
    const arr: Array<MainGasTableDataType> = [];
    const filteredElements = mainGraphData?.filter((item: any) => {
      if (!arr.includes(item.barcode)) {
  ...
  useEffect(() => {
    if (filteredArray.length) dispatch(getSelectedGraphItem(filteredArray[0].barcode));
  }, [filteredArray]);
    ...
    <div className="meters-graph">
      <div className="d-flex justify-content-between align-items-center">
        ...
        <div className="d-flex align-items-center">
          <div className="d-flex align-items-center">
            <button
              onClick={() => dispatch(getSelectedGraphName("usegascount"))}
              className={
