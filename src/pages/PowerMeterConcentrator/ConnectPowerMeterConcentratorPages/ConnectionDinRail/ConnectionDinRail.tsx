...
import { connectConcentratorDinRail } from "../../../../store/slicesAndThunks/powerConcentrator/dinRail/dinRail.thunk";

export const ConnectionDinRail: FC = () => {
  // hooks
  ...
  const formik = useFormik({
    ...
    initialValues: { ip: "", port: "", deviceId: "", password: "", type: "ONE_PHASE" },
    ...
    onSubmit: async (values: any) => {
      await dispatch(connectConcentratorDinRail(values));
    },
  });
    ...
    <div className="connection-gprs">
      <div className="connection-gprs__header d-flex justify-content-between">
        ...
        <MainButton
          ...
          isSecondary
          onClick={() => history.push("/admin/concentrators/power-meter/concentrator-info?tabValue=info-by-rs&page=1")}
        />
      </div>
      ...
      <Form className="d-flex align-items-start connection-gprs__wrap" onSubmit={formik.handleSubmit}>
        ...
        <div className="mr12px"></div>
        <Form.Group className="operator-character__form-item">
          <Form.Label style={{ fontSize: "16px", lineHeight: "24px", fontWeight: 400 }}>Тип</Form.Label>
          <Form.Control
            as="select"
          ...
          >
            <option value={"ONE_PHASE"}>Однофазный</option>
            <option value={"THREE_PHASE"}>Трехфазный</option>
          </Form.Control>
        </Form.Group>
