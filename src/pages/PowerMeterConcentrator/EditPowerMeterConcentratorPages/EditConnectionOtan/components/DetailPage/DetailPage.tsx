...
import { changeOtanDescription } from "../../../../../../store/slicesAndThunks/powerConcentrator/otan/otan.thunk";
...
export const DetailPage: FC = () => {
  const { otanDetailValues, selectedOtanEditMeterId } = useTypedSelector((state) => state.powerMeterOtanMeter);

  const dispatch = useAppDispatch();

  const [openFirstCalendar, setOpenFirstCalendar] = useState(false);
  const [startDate, setStartDate] = useState<MaterialUiPickersDate | null | string>(null);

  useEffect(() => {
    otanDetailValues?.verificationDate &&
      setStartDate(otanDetailValues.verificationDate === "" ? null : otanDetailValues?.verificationDate);
  }, [otanDetailValues]);
  ...
  const formik = useFormik({
    ...
    initialValues: {
      manufacturer: otanDetailValues?.manufacturer || "",
      setUpOrganization: otanDetailValues?.setUpOrganization || "",
      meterName: otanDetailValues?.meterName || "",
      filterMeterName: "",
    },
    onSubmit: async (values) => {
      const body1 = {
        id: selectedOtanEditMeterId ?? "",
        type: 'otan',
      ...
      await dispatch(changeOtanDescription(body));
    },
  });
