...
export const Counterparty: FC<PropsType> = () => {
  ...
  const { otanDetailValues, otanStep1Values } = useTypedSelector((state) => state.powerMeterOtanMeter);

  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      personalAccountNumber: otanDetailValues?.userInfo?.personalAccountNumber || "",
      roleName: otanDetailValues?.userInfo?.roleName || "",
      organizationName: otanDetailValues?.userInfo?.organizationName || "",
      lastname: otanDetailValues?.userInfo?.lastname || "",
      firstname: otanDetailValues?.userInfo?.firstname || "",
      fathersname: otanDetailValues?.userInfo?.fathersname || "",
      position: otanDetailValues?.userInfo?.position || "",
      phoneNumber: otanDetailValues?.userInfo?.phoneNumber || "",
      email: otanDetailValues?.userInfo?.email || "",
      city: otanDetailValues?.userInfo?.city || "",
      district: otanDetailValues?.userInfo?.district || "",
      street: otanDetailValues?.userInfo?.street || "",
      house: otanDetailValues?.userInfo?.house || "",
      flat: otanDetailValues?.userInfo?.flat || "",
    },
    onSubmit: async (values) => {
      const body = {
        ...values,
        ip: otanDetailValues?.ip || "",
        port: otanDetailValues?.port || "",
        deviceId: otanDetailValues?.deviceId || "",
      };

      await dispatch(changeOtanUserInfo(body));
    },
  });
      ...
      <div className="edit-gas-udp-concentrator-counterparty__header d-flex">
        ...
        <div className="edit-gas-udp-concentrator-counterparty__btn" style={{ marginTop: "31px" }}>
          <MainButton
            ...
            onClick={(e) => {
              e.preventDefault();
              dispatch(otanPersonalAccountSearch(formik.values.personalAccountNumber));
            }}
          />
        </div>
      </div>
