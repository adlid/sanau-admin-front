...
import { setSelectedEditOtanMeterId } from "../../../../../../../store/slicesAndThunks/powerConcentrator/otan/otan.slice";

type PropsType = {
  meterData: OtanListResponseType;
  openSidebar: any;
  setSelecteditem: any;
...
export const ConcentratorMeterItem: FC<PropsType> = (props) => {
  ...
  const editMeter = async () => {
    await dispatch(setSelectedEditOtanMeterId(meterData.id))
    history.push(`/admin/concentrators/power-meter/add-concentrator/connection-by-otan/edit/${meterData.id}`)
  }
      ...
      <Row style={{ wordWrap: "break-word", padding: "16px" }}>
        ...
        <Col xl={6}>
          <Row>
            ...
            <Col xl={2} lg={2}>
              {meterData.userInfo ? <Checked /> : <NotChecked />}
              {/* Привязан */}
            </Col>
