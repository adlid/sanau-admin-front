...
} from "../../../../../store/slicesAndThunks/powerConcentrator/otan/otan.slice";
import {
...
import { ConcentratorOtanItemSideBar } from "./components/ConcentratorItemSideBar/ConcentratorOtanItemSideBar";
import { GroupDetailListHeader } from "./components/GroupDetailListHeader";

export const InfoByOtan: FC = () => {
  // hooks
  ...
  const { isFetchingModal, selectedAllMeters } = useTypedSelector((state) => state.powerMeterOtanMeter);

  const { otanMetersList, selectedMeters, foldersList, folderLoading, selectedFolderId } = useTypedSelector((state) => state.powerMeterOtanMeter);
  ...
  const onDeactivateMeterClick = async () => {
    ...
    await dispatch(deactivateOtanMetersThunk(selectedMeters));
    dispatch(isFetchingModalAC(false));
  };

  const onActivateMeterClick = async () => {
    dispatch(isFetchingModalAC(true));
    await dispatch(activateOtanMetersThunk(selectedMeters));
    dispatch(isFetchingModalAC(false));
  };

  const setFolderId = (folderId: string) => {
    dispatch(setSelectedFolderIdOtan(folderId))
    const id = foldersList.findIndex(item => item.id === folderId)
    if (id !== -1) {
      setFolder(foldersList[id].name)
      dispatch(getOtanMetersList({ text: inputValue, queryType: categoryValue, folderId }));
    }
  }

  useEffect(() => {
    dispatch(getOtanMetersList({ text: inputValue, queryType: categoryValue, folderId: "" }));
    dispatch(getFoldersOTAN({ text: "" }))

    return () => {
      dispatch(setSelectedFolderIdOtan(""))
      dispatch(resetFolderListOtan())
    }
  }, []);
    ...
    <>
      <Drawer anchor={"right"} open={sideBarOpen} onClose={sideBarToggle(false)} classes={{ paper: "table-drawer" }}>
        <div style={{ width: "450px" }}>
          <ConcentratorOtanItemSideBar close={sideBarToggle(false)} selectedItem={selectedItem} />
        </div>
      </Drawer>
          ...
          <div className="info-concentrators-list">
            <div className="info-concentrators-list__header">
              ...
              <AddButton
                ...
                onClick={() => history.push("/admin/concentrators/power-meter/createGroup?meterType=otan")}
              />
            </div>
              ...
              <div className="d-flex">
                <InputGroup className="info-by-bluetooth__filter">
                  ...
                  <InputGroup.Prepend>
                    <div
                      onClick={() => {
                        dispatch(getOtanMetersList({ text: inputValue, queryType: categoryValue, folderId: selectedFolderId }))
                        // setPage(1);
      ...
      >
        <DeactivateConcentratorMeterPopup
          ...
          selectedConcentratorMeters={otanMetersList}
