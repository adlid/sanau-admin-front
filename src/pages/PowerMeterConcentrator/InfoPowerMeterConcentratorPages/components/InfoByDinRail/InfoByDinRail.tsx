...
import { ConcentratorItemSideBar } from "./components/ConcentratorItemSideBar/ConcentratorItemSideBar";
...
} from "../../../../../store/slicesAndThunks/powerConcentrator/dinRail/dinRail.slice";
import {
...
export const InfoByDinRail: FC = () => {
  ...
  const { isFetchingModal, selectedAllMeters } = useTypedSelector((state) => state.powerMeterDinRailMeter);

  const { dinRailMetersList, selectedMeters, foldersList, folderLoading, selectedFolderId } = useTypedSelector((state) => state.powerMeterDinRailMeter);
  ...
  const onDeactivateMeterClick = async () => {
    ...
    await dispatch(deactivateDinRailMetersThunk(selectedMeters));
    dispatch(isFetchingModalAC(false));
  };

  const onActivateMeterClick = async () => {
    dispatch(isFetchingModalAC(true));
    await dispatch(activateDinRailMetersThunk(selectedMeters));
    dispatch(isFetchingModalAC(false));
  };

  const setFolderId = (folderId: string) => {
    dispatch(setSelectedFolderIdDinRail(folderId))
    const id = foldersList.findIndex(item => item.id === folderId)
    if (id !== -1) {
      setFolder(foldersList[id].name)
      dispatch(getDinRailMetersList({ text: inputValue, queryType: categoryValue, folderId }));
    }
  }

  useEffect(() => {
    dispatch(getDinRailMetersList({ text: inputValue, queryType: categoryValue, folderId: "" }));
    dispatch(getFoldersDinRail({ text: "" }))

    return () => {
      dispatch(setSelectedFolderIdDinRail(""))
      dispatch(resetFolderListDinRail())
    }
  }, []);
    ...
    <>
      <Drawer anchor={"right"} open={sideBarOpen} onClose={sideBarToggle(false)} classes={{ paper: "table-drawer" }}>
        <div style={{ width: "450px" }}>
          <ConcentratorItemSideBar close={sideBarToggle(false)} selectedItem={selectedItem} />
        </div>
      </Drawer>
          ...
          <div className="info-concentrators-list">
            <div className="info-concentrators-list__header">
              ...
              <AddButton
                ...
                onClick={() => history.push("/admin/concentrators/power-meter/createGroup?meterType='dinrail")}
              />
            </div>
              ...
              <div className="d-flex">
                ...
                <InputGroup className="info-by-bluetooth__filter">
                  ...
                  <InputGroup.Prepend>
                    <div
                      onClick={() => {
                        dispatch(getDinRailMetersList({ text: searchValue, queryType: categoryValue, folderId: selectedFolderId }));
                      }}
      ...
      >
        <DeactivateConcentratorMeterPopup
          ...
          selectedConcentratorMeters={dinRailMetersList}
